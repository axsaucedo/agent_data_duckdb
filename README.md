...
> session metadata in a sibling `summary.json`. `encrypted_content` is never
> read. Token usage is **not** on `chat_history` lines — it comes from the
> sibling `updates.jsonl` (`sessionUpdate == "turn_completed"` → `usage`).
> Grok also populates `reasoning_tokens` (other providers leave it `NULL`).
...
> `cache_creation_tokens` is never set for Grok. No dedicated `reasoning_effort`
> column (`stop_reason` holds effort for `source='grok'`).
